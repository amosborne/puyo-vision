    ...
    for b_idx, b in enumerate(boards[:-2]):
        for r_idx, r_puyos in enumerate(b):
            for c_idx, puyo in enumerate(r_puyos):
                nextpuyos = [
                    x[r_idx][c_idx] for x in boards[b_idx + 1 : b_idx + stabilization]
                ]
...
def findLastFlicker(clf, idxrange):
    ...
    for row, col in np.ndindex(boards[0].shape):
        ...
        for idx in range(idxrange[0], idxrange[1] - flicker_length):
            ...
            if isflicker:
                if idx > (
                    frameno + flicker_length + 10
                ):  # this is the latest flicker (padded), make a new set
                    frameno = idx
                    flicker = [(row, col, flicker_clr)]
                elif idx < frameno:  # this flicker is earlier
                    continue
                elif (
                    idx == frameno and flicker is not None
                ):  # this flicker is at the same time, add to the set
...
def processClf(clf):
    ...
    transition_boards = getTransitionBoards(
        stable_clf, transition_frames
    )  # TODO: Convert previous code to numpy.
